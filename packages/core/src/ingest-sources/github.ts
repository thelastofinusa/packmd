...
import { GeneratorOptions, PackmdResult, RawFile, SkippedFile } from "../types"
import { DEFAULT_IGNORES, defaultParserOptions } from "../utils/defaults"
...
export async function fetchGithubRepo(
  ...
  onProgress(`Inspecting repository ${owner}/${repo}..`)
  ...
  if (segments.length > 0) {
    onProgress("Resolving repository reference..")
  ...
  onProgress(`Scanning repository contents (${ref})..`)
  ...
  for (let i = 0; i < limited.length; i += batchSize) {
    ...
    onProgress(
      `Reading files (${Math.min(i + batchSize, limited.length)}/${limited.length})..`
    )
  ...
  onProgress(`Processed ${files.length} files (${skipped.length} skipped)..`)
  ...
  const sourceUrl = `https://github.com/${owner}/${repo}${subpath ? `/tree/${ref}/${subpath}` : ""}`
  const title = `${headerLabel}${refSuffix}`

  const markdown = generateMarkdown(
    { title, sourceUrl, files, tree },
    {
